...
const InviteRecruiterModal = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchAvailableRecruiters = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5000/api/hr/recruiters/available', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/hr/recruiters/${formData.recruiterId}/invite`, {
        method: 'POST',
        headers: { 
