  ...
  const fetchAllReportsData = async () => {
    try {
      ...
      const [statsResponse, timelineResponse, internshipsResponse, trendsResponse] = await Promise.all([
        fetch('https://internhub-backend-d879.onrender.com/api/admin/dashboard/stats', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch(`https://internhub-backend-d879.onrender.com/api/admin/reports/timeline?range=${dateRange}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('https://internhub-backend-d879.onrender.com/api/admin/internships?limit=1000', {
          headers: { 'Authorization': `Bearer ${token}` }
        }),
        fetch('https://internhub-backend-d879.onrender.com/api/admin/reports/trends', {
          headers: { 'Authorization': `Bearer ${token}` }
        })
      ]);
