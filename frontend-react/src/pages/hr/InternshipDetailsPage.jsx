    ...
    const loadInternshipData = async () => {
        try {
            ...
            const [intResponse, appsResponse] = await Promise.all([
                fetch(`https://internhub-backend-d879.onrender.com/api/hr/internships/${id}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json()),
                fetch(`https://internhub-backend-d879.onrender.com/api/hr/internships/${id}/applications`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json())
            ]);
