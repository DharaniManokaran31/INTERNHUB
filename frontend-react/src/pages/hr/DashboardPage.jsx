    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const [profileRes, statsRes, recruitersRes, activityRes] = await Promise.all([
                fetch('https://internhub-backend-d879.onrender.com/api/recruiters/profile', {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json()),
                fetch('https://internhub-backend-d879.onrender.com/api/hr/dashboard', {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json()),
                fetch('https://internhub-backend-d879.onrender.com/api/hr/recruiters', {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json()),
                fetch('https://internhub-backend-d879.onrender.com/api/hr/activity/recent', {
                    headers: { 'Authorization': `Bearer ${token}` }
                }).then(res => res.json())
            ]);
